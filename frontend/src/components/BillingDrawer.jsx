...
export default function BillingDrawer({ open, onClose }) {
  ...
  const handleUpgrade = async (plan) => {
    try {
      ...
      // order details
      const orderDetails = await createOrder(plan);


      // sent to razorPay window for pop-up
      const options = {
        key: import.meta.env.VITE_RAZORPAY_KEY,
        amount: orderDetails.order.amount,
        currency: orderDetails.order.currency,
        name: "orbit",
        description: `${orderDetails.plan.name} Plan`,
        order_id: orderDetails.order.id,
        handler: async (response) => { // calls verifyPayment
          try {
            const { orderDetails } = await api.post(
